...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const handleNova = async () => {
    const m = await nova();
    mostrarToast(`Mensagem "${m.titulo || 'Sem título'}" criada`, 'sucesso');
  };
  ...
  return (
    <div className="flex h-full w-full bg-paper">
          ...
          {sidebarAberta && (
            <div className="flex items-center gap-2.5 pl-1 animate-fade-in">
              ...
              <div className="leading-tight">
                ...
                <div className="text-[10.5px] uppercase tracking-[0.12em] text-ink-500">v0.1 · MVP</div>
              </div>
            </div>
          )}
      ...
      <main className="flex flex-1 flex-col overflow-hidden">
        <GlobalSearch value={buscaAtalho} onChange={setBuscaAtalho} />
